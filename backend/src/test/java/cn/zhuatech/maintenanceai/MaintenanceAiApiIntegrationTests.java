 ...
 */
@SpringBootTest @AutoConfigureMockMvc class MaintenanceAiApiIntegrationTests {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanReadShopfloorDashboard()throws Exception{mvc.perform(get("/api/shopfloor/dashboard").header("Authorization","Bearer "+operatorToken)).andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.metrics[0].label").value("受监测设备"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanPredictMaintenanceRisk()throws Exception{mvc.perform(post("/api/ai/maintenance/predict").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"assetCode\":\"CNC-07\",\"vibrationMmPerSecond\":8.2,\"temperatureCelsius\":91,\"failureEvents30Days\":4,\"healthIndex\":42,\"runtimeHoursSinceService\":3800,\"sensorCoveragePercent\":92}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.riskLevel").value("CRITICAL")).andExpect(jsonPath("$.data.maintenanceWithinHours").value(8));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
